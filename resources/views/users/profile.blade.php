...
@section('content')
      <div class="profile">
        ...
        <div class="perfilBasico">
          <img src="/storage/avatares/{{ Auth::user()->avatar}}" alt="foto de perfil" class="foto-perfil">
          <div class="user-datos">
            <h2 class="NombreC">{{ Auth::user()->name }}</h2>
            <h4 class="emailU">{{ Auth::user()->email }}</h4>
            <div class="boton">
              <a href="/editProfile"><button class="boton1" type="button" name="button">Edit</button></a>
            </div>
          </div>
        </div>
